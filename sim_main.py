...
import os
import json
from helper_func.principal_component_analysis import perform_pca, plot_explained_variance_ratio
...
n, d, nrep, rho, eta = 1000, 8000, 100, 0, 0
prop_confounders=0.02
prop_predictors=0.02
prop_instruments=0.02
scenario_num = 4
run_PCA = False  # whether to run PCA on covariates when rho !=0
...
config = {
    ...
    "rho": rho,
    "eta": eta,
    'prop_confounders': prop_confounders,
    'prop_predictors': prop_predictors,
    'prop_instruments': prop_instruments,
...
# Create PCA directory if needed
if rho !=0 and run_PCA==True:
    pca_dir = os.path.join(base_dir, "pca")
    os.makedirs(pca_dir, exist_ok=True)
    
for rep in tqdm(range(nrep), desc="Simulation Progress", ncols=80):
    df = generate_synthetic_dataset(n=n, d=d, rho=rho, eta=eta, 
                                    prop_confounders=prop_confounders, prop_predictors=prop_predictors, prop_instruments=prop_instruments, 
                                    scenario_num=scenario_num, rep=rep, save_dir=data_dir)

    if rho !=0 and run_PCA==True:
        scaler = StandardScaler()
        X_scaled = scaler.fit_transform(df.loc[:, df.columns.str.startswith("X")])
        n_components = min(n,d)  # set a maximum number of components to avoid excessive dimensions
        X_pca, pca_model = perform_pca(X_scaled, n_components=n_components)
        # Create a new DataFrame with PCA components
        pca_cols = [f'PC{i+1}' for i in range(n_components)]
        dfX_pca = pd.DataFrame(X_pca, columns=pca_cols)
        # Combine with A and Y
        df_pca = pd.concat([df[['A', 'Y']].reset_index(drop=True), dfX_pca.reset_index(drop=True)], axis=1)
        # Plot and save explained variance ratio
        evr_plot_path = os.path.join(pca_dir, f"explained_variance_ratio_rep{rep}.png")
        plot_explained_variance_ratio(pca_model, save_path=evr_plot_path)
        
        # -----------------------------------------------------------------------------
        # Run OAL and IPW methods on PCA covariates
        # -----------------------------------------------------------------------------
        ate_oal_pca, _, _, _, _, _ = calc_outcome_adaptive_lasso(
        df_pca["A"], df_pca["Y"], df_pca[pca_cols], rep,
        plot=(rep < 5), 
        ...
        # Run IPW with PCA covariates
        ate_ipwx_pca, selected_mask_ipwx_pca = calc_ate_vanilla_ipw(df_pca["A"], df_pca["Y"], df_pca[pca_cols], return_selection=True)
    ...
    # ---------------------------------------------------------------------
    # --- Run Outcome Adaptive LASSO with original covariates ---
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # --- IPWX: Run IPW-based methods with ALL covariates ---
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # --- Conf: Run IPW-based methods with confounders ---
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # --- Targ: Run IPW-based methods with confounders and predictors ---
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # --- PontConf: Run IPW-based methods with confounders, predictors, and instruments ---
    # ---------------------------------------------------------------------
    ...
    # Save ATEs
    if rho !=0 and run_PCA==True:
        res_dict["ate"].extend([ate_oal, ate_oal_pca, ate_ipwx, ate_ipwx_pca, ate_conf, ate_targ, ate_pot_conf])
        res_dict["method"].extend(["OAL", "OAL_PCA", "IPWX", "IPWX_PCA", "Conf", "Targ", "PotConf"])
        res_dict["rep"].extend(7 * [rep])
    else:
...
plot_wamd_before_after_grid(wamd_dir=wamd_dir, n_reps=5, n_lambdas=9, figsize=(18, 10))
